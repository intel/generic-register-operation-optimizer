...
A write function wrapped with `write_only` in this way _cannot_ have an `id_spec` that
provides its identity bits.
...
Because writing happens to a register, not an individual field, it is also an
error to write without "covering" all the bits marked as write-only. Those bits
...
// compile error: "Attempting to write but missing a write-only field: f1"
----
...
// compile error: "Write to register reg containing write-only bits which are not provided"
----
