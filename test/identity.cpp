...
TEST_CASE("all specs fulfil mask_spec", "[identity]") {
    STATIC_CHECK(groov::mask_spec<groov::m::zero>);
    STATIC_CHECK(groov::mask_spec<groov::m::one>);
    STATIC_CHECK(groov::mask_spec<groov::m::any>);
}
...
TEST_CASE("replace write function has no identity", "[identity]") {
    STATIC_CHECK(groov::write_function<groov::w::replace>);
    STATIC_CHECK(not groov::identity_write_function<groov::w::replace>);
}

TEST_CASE("ignore write function has any identity", "[identity]") {
    STATIC_CHECK(groov::write_function<groov::w::ignore>);
    STATIC_CHECK(std::is_same_v<groov::w::ignore::id_spec, groov::m::any>);
}

TEST_CASE("write one to set/clear write function has zero identity",
          "[identity]") {
    STATIC_CHECK(groov::write_function<groov::w::one_to_set>);
    STATIC_CHECK(groov::write_function<groov::w::one_to_clear>);
    STATIC_CHECK(std::is_same_v<groov::w::one_to_set::id_spec, groov::m::zero>);
    STATIC_CHECK(
...
TEST_CASE("write zero to set/clear write function has one identity",
          "[identity]") {
    STATIC_CHECK(groov::write_function<groov::w::zero_to_set>);
    STATIC_CHECK(groov::write_function<groov::w::zero_to_clear>);
    STATIC_CHECK(std::is_same_v<groov::w::zero_to_set::id_spec, groov::m::one>);
    STATIC_CHECK(
