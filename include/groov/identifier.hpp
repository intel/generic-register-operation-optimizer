...
template <register_tag R> struct register_id_t;
...
template <field_tag F> struct field_id_t;

template <typename T>
concept field_id = detail::is_instance_of_template_v<field_id_t, T>;

template <register_id R, field_id F> struct field_path_t;

template <typename T>
concept field_path = detail::is_instance_of_template_v<field_path_t, T>;

template <typename I, typename T> struct with_value_t;

template <typename T>
concept field_with_value =
    detail::is_instance_of_template_v<with_value_t, T> and
    field_id<typename T::id_t>;
...
constexpr auto operator/(R, with_value_t<F, T> w)
    -> with_value_t<field_path_t<R, F>, T> {
    return {field_path_t<R, F>{}, w.value};
}

namespace literals {
template <class T, T... chars> struct id_string {};
